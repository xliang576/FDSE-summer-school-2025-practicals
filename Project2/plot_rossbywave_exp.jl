...
### TODO: oblem with uv_prime plotting
...
eps_ic = FieldTimeSeries(filename * ".jld2", "uv_prime", iterations = 0)
...
anim = @animate for (i, iter) in enumerate(iterations)
    ...
    eps_xy = file_xy["timeseries/uv_prime/$iter"][1, :, 1];
    ...
    eps_title = @sprintf("uv_prime, t = %s days", round(t/1day));
    ...
    eps_xy_plot = Plots.plot(eps_xy, yeps/1e3; color = :balance, xlabel = "y (km)", ylabel="uv_prime",aspect_ratio = :equal, title=c_title, fontsize=14);  

    plot(u_xy_plot, c_xy_plot, eps_xy_plot, layout = (1, 3), size = (1300, 600))
